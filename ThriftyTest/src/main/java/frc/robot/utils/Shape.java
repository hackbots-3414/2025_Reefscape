...
public class Shape {
  ...
  public static Shape fromUnsortedVertices(List<Translation2d> unsortedVertices, String name) {
    ...
    mutableList.sort(
        Comparator.comparingDouble(point -> Math.atan2(point.getY() - cy, point.getX() - cx)));
  ...
  public boolean isPointInside(Translation2d point) {
    ...
    for (int i = 0; i < numVertices; i++) {
      ...
      Translation2d end = m_vertices.get((i + 1) % numVertices); // rolls over to 1 after last
                                                                 // vertex
      ...
      if ((y > y1) != (y > y2)) { // if both true? not within y1-y2 bounds
        ...
        if (x < intersectionPoint) { // we only care if its to the RIGHT (when point less than
                                     // intersection point)
          crossings++;
        }
      }
  ...
  public static Shape flipHotdog(Shape shape, String name) {
    ...
    return new Shape(shape.getVertices().stream()
        .map(oldTranslation -> new Translation2d(oldTranslation.getX(),
            FieldConstants.k_fieldWidth.in(Meters) - oldTranslation.getY()))
        .collect(Collectors.toList()), name);
  }

  public Shape flip() {
    return new Shape(this.getVertices().stream()
        .map(oldTranslation -> new Translation2d(
            FieldConstants.k_fieldLength.in(Meters) - oldTranslation.getX(),
            FieldConstants.k_fieldWidth.in(Meters) - oldTranslation.getY()))
